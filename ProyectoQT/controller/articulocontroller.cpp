...
void ArticuloController::getCantArticulosByCategoria(int id){
	QJsonObject jsonObject;
	jsonObject.insert("jsonrpc", "2.0");
	jsonObject.insert("method", "call");
	jsonObject.insert("id", 970248153);
	...
	args.append("1234");
	args.append("simarropop.articulo");
	args.append("search_count");

	QJsonArray criteria;
	criteria.append(QJsonArray {"categoria", "=", id});
...
void ArticuloController::slotPeticionCant(QNetworkReply* reply){
 if (reply->error() != QNetworkReply::NoError) {
            qDebug() << "Error: " << reply->errorString();
        } else  if (reply->isFinished()){
...
int ArticuloController::getCantidad(){
	QJsonObject jsonResponse = responseData.object();
            

           return jsonResponse.value("result").toInt();
